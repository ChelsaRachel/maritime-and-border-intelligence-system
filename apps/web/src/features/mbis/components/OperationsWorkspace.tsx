...
import { ImmigrationIntegrationSummary } from '@/features/mbis/components/ImmigrationIntegrationSummary'
...
export function OperationsWorkspace({ module }: { module: TWorkspaceModule }) {
  ...
  const renderAircraft = () => {
    ...
    return <div className="workspace-grid workspace-grid--aircraft">
      ...
      <Panel title="Flight Profile" eyebrow="Altitude and speed history" className="span-8"><div className="flight-chart">{(() => { const maxAlt = Math.max(...aircraft.history.map((h: any) => h.altitude), 1); const maxSpeed = Math.max(...aircraft.history.map((h: any) => h.speed), 1); return aircraft.history.map((item: any) => <div key={item.time}><i style={{ height: `${Math.max(3, (item.altitude / maxAlt) * 94)}%` }} /><b style={{ height: `${Math.max(3, (item.speed / maxSpeed) * 94)}%` }} /><span>{item.time}</span></div>) })()}</div></Panel>
  ...
  const renderWarning = () => <div className="workspace-grid workspace-grid--warning">
    ...
    <Panel title="7-day Alert Trend" eyebrow="Five-minute refresh simulation" className="span-6"><div className="trend-lines">{(() => { const trendMax = Math.max(...operations!.threat.trend.flatMap((t: any) => [t.maritime, t.border]), 1); return operations!.threat.trend.map((item: any) => <div key={item.date}><i style={{ height: `${Math.max(3, (item.maritime / trendMax) * 94)}%` }} /><b style={{ height: `${Math.max(3, (item.border / trendMax) * 94)}%` }} /><span>{item.date}</span></div>) })()}</div></Panel>
  ...
  const renderThreat = () => <div className="workspace-grid workspace-grid--threat">
    ...
    <Panel title="Regional Risk Score" eyebrow="Top 10" className="span-4 threat-analysis-panel threat-regional-panel"><PerfectScrollArea className="threat-regional-scroll" options={{ wheelPropagation: false }}><DataTable columns={[{key:'name',label:'REGION'},{key:'score',label:'SCORE'},{key:'trend',label:'7D Δ'}]} rows={operations!.threat.regions} /></PerfectScrollArea></Panel>
    <Panel title="Threat Drivers" eyebrow="Weighted contribution" className="span-4 threat-analysis-panel threat-driver-panel"><div className="factor-list">{operations!.threat.drivers.map((item: any) => <label key={item.name}><span>{item.name}</span><progress max="100" value={item.score} /><b>{item.score}</b></label>)}</div></Panel>
    <Panel title="Predictive Model / 3–24h" eyebrow="Deterministic fixture forecast" className="span-4 threat-analysis-panel threat-forecast-panel"><DataTable columns={[{key:'region',label:'REGION'},{key:'probability',label:'PROBABILITY %'},{key:'level',label:'LEVEL'},{key:'confidence',label:'CONFIDENCE'}]} rows={operations!.threat.forecast} /></Panel>
