...
export function OperationsWorkspace({ module }: { module: TWorkspaceModule }) {
  ...
  const renderBorder = () => (
    <div className="workspace-grid workspace-grid--border">
      ...
      <Panel title="Border Incident Timeline" eyebrow="Last 7 days" className="span-7"><div className="timeline-bars">{border!.timeline.map((item: any) => <div key={item.date}><span>{item.date}</span><i style={{ height: `${18 + item.high * 13}px` }} /><b>{item.low + item.medium + item.high}</b></div>)}</div></Panel>
